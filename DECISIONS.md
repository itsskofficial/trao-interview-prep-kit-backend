...
It was first built on Brave Search and never exercised, because the project had no key and that service's free plan was not one this project could rely on. LangSearch replaced it: its free plan (1,000 searches a day, 60 a minute) fits the brief's rule that everything must work on a genuinely free tier, so the live application can actually use it. One optional provider, not two.
