...
## 38. An optional web search, off unless there is a key for it

Hacker News and Stack Exchange rarely discuss how a small company interviews; most of that is on blogs and forums that a general web search finds. It needs a key, and an evaluator's clean clone has only a model key, so it cannot be the default. With `LANGSEARCH_API_KEY` set, one web search per kit is added to the two keyless sources; without it nothing changes.

It was first built on Brave Search and never exercised, because the project had no key and that service's free plan was not one this project could rely on. LangSearch replaced it: its free plan (a daily allowance that resets at midnight UTC; a kit makes one search) fits the brief's rule that everything must work on a genuinely free tier, so the live application can actually use it. One optional provider, not two.

Its results are strangers' text like any other and go through the same filters: a hit must name the company and talk about interviewing, only http(s) addresses are kept, and nothing reaches a kit without the quoted-evidence check of decision 30. A response that is not the documented shape is logged as unreadable rather than as "nothing found". The fetcher gained request headers and a JSON body for this. Both are sent only to the origin that was asked for; a POST that is redirected is refused rather than re-sent elsewhere (tested over real HTTP); neither is ever recorded in a run trace; and the key is never part of an address.

**Why not the model's own web search.** Gemini can search Google itself ("grounding"), which would need no second key. It was tried with this project's free-tier key on 2026-09-20: a plain call to `gemini-3.5-flash-lite` succeeds, and the same call with the search tool is refused as over quota, as it is on `gemini-3.5-flash`. So it is not available on the free tier the brief requires. It would also fit the design worse than a plain search does: grounding returns the model's own summary with redirect links rather than the sources' text, so there would be nothing to check a quote against without fetching the cited pages anyway, and it would disappear whenever the fallback provider took over.
