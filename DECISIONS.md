...
Hacker News and Stack Exchange rarely discuss how a small company interviews; most of that is on blogs and forums that a general web search finds. It needs a key, and an evaluator's clean clone has only a model key, so it cannot be the default. With `BRAVE_SEARCH_API_KEY` set, a web search is added to the two keyless sources; without it nothing changes. Its results are strangers' text like any other and go through the same filters: a hit must name the company and talk about interviewing, only http(s) addresses are kept, and nothing reaches a kit without the quoted-evidence check of decision 30. The fetcher gained request headers for this. They are sent only to the origin that was asked for, so a redirect cannot walk a key off to another host (tested over real HTTP), they are never recorded in a run trace, and the key is never part of an address. It has not been exercised against the live service, because this project has no key for it; the tests use the documented response shape.
