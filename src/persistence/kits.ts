...
export function kitRepository(db: Database) {
  ...
  return {
    async create(userId: ObjectId, kit: Kit, fingerprint: string): Promise<StoredKit> {
      const now = new Date();
      const doc: KitDoc = { _id: new ObjectId(), userId, kit, fingerprint, counters: highestIds(kit), createdAt: now, updatedAt: now };
...
function toStored(doc: KitDoc): StoredKit {
  return { id: doc._id.toHexString(), kit: doc.kit, createdAt: doc.createdAt.toISOString(), updatedAt: doc.updatedAt.toISOString() };
}
